...
export const createToolSchema = z.object({
  body: z
    .object({
      ...
      // accessLink: required for regular tools, optional for packages
    ...
    .superRefine((data, ctx) => {
      // Regular tools must have a valid accessLink
      if (!data.isPackage) {
        if (!data.accessLink || data.accessLink.trim() === "") {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "accessLink is required for tools",
            path: ["accessLink"],
          });
        }
      }
...
export const updateToolSchema = z.object({
  ...
  body: z.object({
    ...
    accessLink:       z.string().url().optional().or(z.literal("")),
